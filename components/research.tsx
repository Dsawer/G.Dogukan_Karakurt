  ...
  return (
    <article className="overflow-hidden rounded-lg border border-border-soft bg-[color:var(--surface)]/40 shadow-[inset_3px_0_0_var(--accent)]">
      <header className="space-y-2 px-6 pt-6">
        ...
        <h3 className="text-balance text-[19px] font-bold leading-snug tracking-tight text-foreground">
          {research.title}
        </h3>
      ...
      <div className="space-y-6 px-6 py-5">
        <p className="text-pretty text-[13.5px] leading-relaxed text-slate">
          {research.pitch}
        </p>
        ...
        <figure className="overflow-hidden rounded-md border border-border-soft">
          ...
          />
          <figcaption className="border-t border-border-soft bg-[color:var(--background)]/40 px-3 py-2 font-mono text-[10px] uppercase tracking-widest text-slate-dim">
        ...
        <div className="space-y-px">
          {research.sections.map((s, i) => (
            <div
              key={s.label}
              className={i === 0 ? "pt-0" : "border-t border-border-soft pt-3"}
            >
              <h4 className="mb-1 font-mono text-[10.5px] uppercase tracking-widest text-accent">
                {s.label}
              </h4>
              <p className="pb-3 text-[13px] leading-relaxed text-slate">{s.body}</p>
            </div>
          ))}
        </div>
